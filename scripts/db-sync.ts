...
async function syncDatabase() {
  try {
    ...
    // Add missing "price" column to order_extra_charges (table may exist from older schema without it)
    try {
      const tableResult = await tempConnection.query(
        `SELECT table_name FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'order_extra_charges' LIMIT 1`
      );
      ...
      if (tableRow?.table_name) {
        const colResult = await tempConnection.query(
          `SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'order_extra_charges' AND column_name = 'price' LIMIT 1`
        );
        const hasPrice = (colResult as any)?.rows?.[0] ?? (colResult as any)?.[0];
        if (!hasPrice) {
          await tempConnection.query(`
            ALTER TABLE "order_extra_charges"
            ADD COLUMN IF NOT EXISTS "price" numeric(10,2) NOT NULL DEFAULT 0
          `);
          console.log('✅ Added missing "price" column to order_extra_charges');
        }
      }
    } catch (err: any) {
      if (process.env.NODE_ENV !== 'production') {
        console.warn('⚠️ order_extra_charges add price column:', err?.message ?? err);
      }
    }
    ...
    // Add missing "price" column to service_extras if needed, then set NULL prices to 0
    try {
      const tableResult = await tempConnection.query(
        `SELECT table_name FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'service_extras' LIMIT 1`
      );
      const tableRow = (tableResult as any)?.rows?.[0] ?? (tableResult as any)?.[0];
      if (tableRow?.table_name) {
        const colResult = await tempConnection.query(
          `SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'service_extras' AND column_name = 'price' LIMIT 1`
        );
        const hasPrice = (colResult as any)?.rows?.[0] ?? (colResult as any)?.[0];
        if (!hasPrice) {
          await tempConnection.query(`
            ...
            ADD COLUMN IF NOT EXISTS "price" numeric(10,2) NOT NULL DEFAULT 0
          `);
          console.log('✅ Added missing "price" column to service_extras');
        } else {
          const updateResult = await tempConnection.query(
            `UPDATE "service_extras" SET "price" = 0 WHERE "price" IS NULL`
          );
          const updateCount = (updateResult as any)?.rowCount ?? (updateResult as any)?.[1] ?? 0;
          if (updateCount > 0) {
            console.log(`✅ Set price = 0 for ${updateCount} service_extras row(s) that had NULL price`);
          }
        }
      }
    } catch (err: any) {
      if (process.env.NODE_ENV !== 'production') {
        console.warn('⚠️ service_extras price fix:', err?.message ?? err);
      }
    }
